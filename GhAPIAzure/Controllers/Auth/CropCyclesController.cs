namespace GhAPIAzure.Controllers
{
    ...
    public class CropCyclesController : BaseController
    {
        ...
        // GET: api/Greenhouses
        /// <summary>Gets all crop cycles in all grenehosues for this user</summary>
        /// <remarks></remarks>
        ...
        [SwaggerResponse(HttpStatusCode.Forbidden, "Happens when you try to edit someone else's stuff",
             Type = typeof(ErrorResponse<CropCycle>))]
        public async Task<HttpResponseMessage> PostCropCycles(List<CropCycle> cCyclesRecieved)
        {
            ...
            var IDs = cCyclesRecieved.Select(cc => cc.ID).ToArray();
            var ghIDs = cCyclesRecieved.Select(gh => gh.LocationID).ToArray();

            var Greenhouses =
                await db.Location.Where(gh => ghIDs.Contains(gh.ID)).Include(gh => gh.CropCycles).ToListAsync();
            var cropTypes = await db.CropTypes.ToListAsync();
            var cCyclesDb = Greenhouses.SelectMany(gh => gh.CropCycles).ToList();

            var missingGh = cCyclesRecieved.FirstOrDefault(cc => !Greenhouses.Any(gh => gh.ID == cc.LocationID));
            if (missingGh != null)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest,
                    new ErrorResponse<CropCycle>("This cycle spesifies invalid greenhosue ID", missingGh));
            }
            var missingCrop = cCyclesRecieved.FirstOrDefault(cc => !cropTypes.Any(ct => ct.Name == cc.CropTypeName));
            if (missingCrop != null)
            {
                return Request.CreateResponse(HttpStatusCode.BadRequest,
                    new ErrorResponse<CropCycle>("This cycle spesifies invalid CropType", missingCrop));
            }
            //Check ownership
            ...
            if (null != invalidCycle)
            {
                return Request.CreateResponse(HttpStatusCode.Forbidden,
                    new ErrorResponse<CropCycle>("Can't create a cycle on a greenhosue that does not belong to you",
                        invalidCycle));
            }

            foreach (var cCycleRecieved in cCyclesRecieved)
            {
