namespace GhAPIAzure.Controllers
{
    ...
    public class RelayHistoryController : BaseController
    {
        ...
        /// <summary>Gets historical record of what relays were doing.</summary>
        /// <remarks>Gets historical record of what relays were doing. In the "from" parameter you should
        /// spesify the date when you have last synced with the server. The server will then 'slice' the
        /// datapoints for that day, and only provide you with ones that were produced later than the from date
        /// Timestamp on the day should be midnight of the day when recording FINISHES. It's the end of that
        /// day. All the records attached to that day sould be timestamped before the day!</remarks>
        ...
        {
            var from = new DateTimeOffset(dateTimeTicks, TimeSpan.Zero);
            var RelayHistory =
                await
                    db.RelayHistories.Where(Ch => Ch.Location.PersonId == _UserID && Ch.TimeStamp > from)
        ...
        /// <summary>Accepts a list of RelayHistories you want to edit.</summary>
        /// <remarks> This accepts delta updates. So you can add a RelayHistory that has only one new datapoint
        /// each, and it will just add it on top of what's already in the DB</remarks>
        ...
        [SwaggerResponse(HttpStatusCode.Forbidden, "Happens when you try to edit someone else's stuff",
             Type = typeof(ErrorResponse<RelayHistory>))]
        [SwaggerResponse(HttpStatusCode.NotFound, "Refers to something that doesn;t exist",
             Type = typeof(ErrorResponse<RelayHistory>))]
        public async Task<HttpResponseMessage> PostRelayHistory(List<RelayHistory> rhRecievedList)
        {
            ...
            var relayIDs = rhRecievedList.Select(chR => chR.RelayID).ToList();

            //Get all relevant RelayHistories, they must both exist and belong to this user! 
            var usersRelays =
                await
                    db.Relays.Where(rel => relayIDs.Contains(rel.ID) && rel.Device.Location.PersonId == _UserID)
                        .ToListAsync();

            //If one of the submitted items reffers to a Relay that doesnt exist/belong to user, return error
            foreach (var rHistory in rhRecievedList)
            {
            ...
            var timestamps = rhRecievedList.Select(relHist => relHist.TimeStamp).ToList();
            var relHistDbRawList =
                await
                    db.RelayHistories.Where(
                            relHist => timestamps.Contains(relHist.TimeStamp) && relayIDs.Contains(relHist.RelayID))
                        .ToListAsync();
            var userLocations = await db.Location.Where(loc => loc.PersonId == _UserID).ToListAsync();

            foreach (var relHistRecieved in rhRecievedList)
            {
                var relHistDB =
                    relHistDbRawList.FirstOrDefault(
                        rHist =>
                                rHist.RelayID == relHistRecieved.RelayID && rHist.TimeStamp == relHistRecieved.TimeStamp);
                ...
                else
                {
                    if (relHistDB.LocationID != relHistRecieved.LocationID)
                    {
                        return Request.CreateResponse(HttpStatusCode.Forbidden,
                            new ErrorResponse<RelayHistory>("You are not allowed to change location of dataHistory",
                                relHistRecieved));
                    }

                    relHistDB.DeserialiseData();
                    var chMerged = RelayHistory.Merge(relHistDB, relHistRecieved);
