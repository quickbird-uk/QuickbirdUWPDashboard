...
namespace GhAPIAzure.Controllers
{
    ...
    public class CropTypesController: BaseController
    {
        ...
        /// <summary>
        /// Special. Creates a new Crop
        /// </summary>
        /// <remarks>Creates a new crop. Once created, crops cannot be edited by users. Crops are created by a certain user.
        /// If you are creating a new crop, CreatedBY should be set to the ID of your user. 
        /// Approved flag indicates wether this crop should be shown publicly or should only be displayed to the user that created it
        /// Users cannot directly edit the approved status of a crop</remarks>
        ...
        [SwaggerResponse(HttpStatusCode.Forbidden, "Actions you are not allowed to do", Type = typeof(ErrorResponse<CropType>))]
        public async Task<HttpResponseMessage> PostCropTypes(List<CropType> cTypesRecieved)
        {
            ...
            var invalidName = cTypesRecieved.FirstOrDefault(ct => ct.Name.Length > 240 || String.IsNullOrWhiteSpace(ct.Name));
            if (invalidName !=null)
            {
            ...
            for (int i = 0; i < cTypesRecieved.Count; i++)
            {
                ...
                if (match != null)
                {
                    ...
                    if (cTypesRecieved[i].Approved && match.Approved == false //we don;t let users upgrade the status, but if their version is not approved, we let it slide
                        || cTypesRecieved[i].CreatedBy != match.CreatedBy
                        || cTypesRecieved[i].CreatedAt != match.CreatedAt)
                    { return Request.CreateResponse(HttpStatusCode.Forbidden,
                        new ErrorResponse<CropType>("You are not allowed to edit creation time and approval of crops", cTypesRecieved[i])); }
            ...
            {
                return Request.CreateResponse(HttpStatusCode.Forbidden, 
                    new ErrorResponse<CropType>("All new crops should have approved set ot false, and CreatedBy  should be your user", null));
            }
            else
            {
